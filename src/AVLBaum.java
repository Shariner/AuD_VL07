...
// Methoden zum Suchen
	...
	{
		AVLKnoten<T> temp = wurzel;
		while(temp != null) {
			if(daten.compareTo(temp.getDaten()) < 0){
				temp = temp.getKnotenLinks();
				
			}
			else if(daten.compareTo(temp.getDaten()) > 0) {
				temp = temp.getKnotenRechts();
			}
			else 
				return true;
		}
...
// Methode zum Traversieren
	...
	{
		Deque<AVLKnoten<T>> ausgabe = new LinkedList<AVLKnoten<T>>();
		AVLKnoten<T> next = null;
		String ret = "";
		ausgabe.push(wurzel);
		while(!ausgabe.isEmpty()) {
			...
			if(next.getKnotenRechts() !=null){
				ausgabe.push(next.getKnotenRechts());
				
			}
			if(next.getKnotenLinks() != null) {
				ausgabe.push(next.getKnotenLinks());
			}
